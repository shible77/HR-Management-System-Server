...
export const checkIn = async (req: SessionRequest, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json({ status: true, message: "Check-in successful" });
  } catch (error) {
    throw error;
  }
};
...
export const checkOut = async (req: SessionRequest, res: Response) => {
  try {
    const today = new Date();
    const { attendanceId } = validate(checkOutSchema, {
    ...
    return res
      .status(200)
      .json({ status: true, message: "Check-out successful" });
  } catch (error) {
    throw error;
  }
};
...
export const getAttendanceByDate = async (
  ...
  try {
    const { date, cursor, limit } = validate(getAttendanceByDateSchema, {
      ...req.query,
    });
...
export const getAttendanceByDepartment = async (
  ...
  try {
    const { date, departmentId, cursor, limit } = validate(
      getAttendanceByDepartmentSchema,
...
export const getAttendanceByMonthPerEmployee = async (
  ...
  try {
    const { date, employeeId } = validate(getAttendanceByMonthSchema, {
      ...req.query,
...
export const getAllAttendanceOfMe = async (
  req: SessionRequest,
  res: Response,
) => {
  try {
    const { cursor, limit } = validate(getAllAttendanceOfMeSchema, {
      ...req.query,
    });
    const employeeId = req.employeeId!;
    const records = await db
...
export const getAbsentEmployeeByDate = async (
  ...
  try {
    ...
    const record = await db
      ...
      .innerJoin(
        departments,
        eq(departments.departmentId, employees.departmentId),
      )
      .leftJoin(
        attendance,
        and(
